...
router.route('/').get(function (req, res) {
  Issues.find(function (err, issues){
    if(err){
      ...
      console.log('Retriving issues');
      res.json(issues);
    }
  });
});
// router.route('/issues/:id').post(function (req, res) {
//   // var issues = req.issues;

//   // issues = _.extend(issues, req.body);

//   let issues = new Issues(req.body);

//   issues.save()
//     .then(issues => {
//     res.status(200).json({'issues': 'issues in added successfully'});
//     })
//     .catch(err => {
